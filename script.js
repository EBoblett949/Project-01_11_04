...
function getLocation() {
    ...
    httpRequest.abort();
    httpRequest.open("get", "https://api.zippopotam.us/" + countrySel + "/" + zip, true);
...
function displayData() {
    if (httpRequest.readyState === 4 && httpRequest.status === 200) {
        ...
        city.value = resultData.places[0]["state abbreviation"];
...
if (zip.addEventListener) {
    ...
    zip.addEventListener("onkeyup", checkInput);
}
